...
void buildImageToECR() {
    ...
    withCredentials([usernamePassword(credentialsId: env.AWS_JENKINS_ACC_KEY_ID, passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
        // Before awscli2 installed on Jenkins container       
        ...
        // Before awscli2 installed on Jenkins        
        env.ECR_TOKEN = sh(script:'aws ecr get-login-password', returnStdout: true).trim()
        ...
        echo "Being sure whether the repo is exist"
        sh 'aws ecr create-repository --repository-name $REPO_NAME || true'
        ...
        env.IMAGE_NAME = "${AWS_ACCOUNT}.dkr.ecr.${AWS_REGION}.amazonaws.com/${REPO_NAME}:${APP_VER}-jbn${BUILD_NUMBER}"
...
void deployToKVM() {
    sh "sed -i '/appVersion/c\\appVersion: $APP_VER-jbn$BUILD_NUMBER' helm-chart/Chart.yaml"
